...
const SocialMediaIcon = ({ children }) => (
    <Box sx={{ color: 'white', mx: 1 }}>
        {children}
    </Box>
);

const FooterSection = ({ title, subtitles }) => (
        ...
        <Box sx={{ backgroundColor: 'rgba(0, 89, 188, 1)', color: 'rgba(255, 255, 255, 1)', pb: 2, pt: 5 }}>
            <Grid container spacing={5} sx={{ paddingLeft: "10%", paddingRight: "2%" }}>
                <Grid item xs={12} md={6} mb={2}>
                    ...
                    <Typography variant="body2" sx={{ ...textStyles, fontSize: '16px', lineHeight: '18.75px', marginBottom: '50px', width: '50%', color: 'rgba(255, 255, 255, 1)', paddingTop: '3%', display: { xs: 'none', md: 'block' } }}>
                ...
                <Grid item xs={12} md={6} sx={{ display: { xs: 'none', md: 'block' } }}>
                    <Grid container spacing={2} sx={{ paddingLeft: "20%", paddingTop: "5%" }}>
                        {titles.map((title, index) => (
                            <FooterSection key={index} title={title} subtitles={subtitles} />
                        ))}
                    </Grid>
                </Grid>
            </Grid>
            ...
            <Box display="flex" alignItems="center" justifyContent="space-between" mt={5} borderTop={2} borderColor="rgba(255, 255, 255, 0.2)" pt={2} sx={{ width: '100%', boxSizing: 'border-box', paddingLeft: "10%", paddingRight: "10%" }}>
                ...
                <Box display="flex">
                    <SocialMediaIcon><InstagramIcon /></SocialMediaIcon>
                    <SocialMediaIcon><FacebookIcon /></SocialMediaIcon>
                    <SocialMediaIcon><LinkedInIcon /></SocialMediaIcon>
                </Box>
            </Box>
        </Box>
    );
